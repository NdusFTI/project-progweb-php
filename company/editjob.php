...
if (!$job_id) {
  echo "<script>alert('Job ID tidak ditemukan!'); window.location.href = '../';</script>";
  exit;
}

// Fetch existing job data
$sql = "SELECT jp.*, jc.name as category_name FROM job_postings jp 
...
if (!$job) {
  echo "<script>alert('Job tidak ditemukan!'); window.location.href = '../';</script>";
  exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (empty($errors)) {
    $sql = "UPDATE job_postings SET category_id = ?, title = ?, description = ?, requirements = ?, salary_min = ?, salary_max = ?, salary_text = ?, location = ?, full_address = ?, job_type = ?, experience_required = ?, application_deadline = ?, is_active = ? WHERE id = ?";
    ...
    if ($stmt->execute()) {
      echo "<script>
        alert('Success! Job posting updated successfully!');
        window.location.href = '../';
      </script>";
    } else {
      echo "<script>
        alert('Error! Failed to update job posting. Please try again.');
      </script>";
    }
  } else {
    $error_text = implode('\\n', $errors);
...
<head>
  ...
  <title>SugoiJob - Edit Jobs</title>
...
<body>
  <div class="form-container">
    <div class="form-header">
      ...
      <p>Update informasi pekerjaan untuk menarik kandidat terbaik</p>
    </div>

    <div class="form-content">
      <form action="editjob.php?id=<?php echo $job_id; ?>" method="POST" id="job-form">
        <div class="form-grid">
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input type="text" id="category" name="category" required placeholder="Enter your job category here"
                value="<?php echo htmlspecialchars($job['category_name']); ?>" />
            </div>
            ...
            <div class="form-group">
              ...
              <select id="job_type" name="job_type" required>
                ...
                <option value="Kontrak" <?php echo ($job['job_type'] == 'Kontrak') ? 'selected' : ''; ?>>Kontrak</option>
                <option value="Internship" <?php echo ($job['job_type'] == 'Internship') ? 'selected' : ''; ?>>Internship
                </option>
              </select>
            </div>
          </div>
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <input type="text" id="full_address" name="full_address" required
                ...
                value="<?php echo htmlspecialchars($job['full_address']); ?>" />
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <textarea id="description" name="description" required
              placeholder="Jelaskan peran, tanggung jawab, dan apa yang membuat pekerjaan ini menarik..."><?php echo htmlspecialchars($job['description']); ?></textarea>
          </div>
          ...
          <div class="form-group">
            ...
            <textarea id="requirements" name="requirements" required
              placeholder="Daftar keterampilan, pengalaman, dan kualifikasi penting yang dibutuhkan untuk pekerjaan ini..."><?php echo htmlspecialchars($job['requirements']); ?></textarea>
          </div>
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <select id="status" name="status">
                <option value="active" <?php echo ($job['is_active'] == 1) ? 'selected' : ''; ?>>Active (Accept
                  Applications)</option>
                <option value="draft" <?php echo ($job['is_active'] == 2) ? 'selected' : ''; ?>>Draft (Save for Later)
                </option>
                <option value="paused" <?php echo ($job['is_active'] == 0) ? 'selected' : ''; ?>>Paused (Temporarily
                  Closed)</option>
              </select>
            </div>
